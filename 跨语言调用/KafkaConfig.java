package com.ping.job.manager.config;

import lombok.extern.slf4j.Slf4j;
import com.ping.job.manager.boot.ServerInfo;
...
public class KafkaConfig {
    ...
    private final Integer dftMaxPollIntervalMs = 300*1000;
    @Value("${spring.kafka.consumer.auto-commit-interval}")
    private String autoCommitIntervalMs;

    /**
     * kafka bean注册成功flag bean
     * 用以告知kafka bean已经注册完成
     *
     * DependsOn注解表明，一个bean在所依赖的bean创建完成后才创建
     */
    @Bean(name = "kafkaBeanCreatedFlag")
    @DependsOn({"kafkaTemplate","kfkListenerFacAutoCommit",
            "kfkListenerFacManualCommit","kfkListenerFacAutoCommitDy",
            "kfkListenerFacManualCommitDy"})
    public Boolean kafkaFlag(){
        log.info("kafka bean created completely!");
        return true;
    }
    ...
    public KafkaTemplate kafkaTemplate(){
        ...
        //生产者id，默认前缀为producer
        configs.put(ProducerConfig.CLIENT_ID_CONFIG,"producer-2020");
        //发送失败时的重试次数
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Integer, String>> kfkListenerFacAutoCommit() {
        log.info("kfkListenerFacAutoCommit注册");
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Integer, String>> kfkListenerFacManualCommit() {
        log.info("kfkListenerFacManualCommit注册");
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Integer, String>> kfkListenerFacAutoCommitDy() {
        log.info("kfkListenerFacAutoCommitDy注册");
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Integer, String>> kfkListenerFacManualCommitDy() {
        log.info("kfkListenerFacManualCommitDy注册");
