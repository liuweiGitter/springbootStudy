...
public class LogCenter {
    ...
    private static volatile boolean LOG_MAN_START_FLAG = false;
    ...
    public static void destroyPool(long waitTime){
        log.warn("》》》LogCenter线程池销毁，休眠"+waitTime+"ms等待任务后处理");
        destroyFlag = true;
        LOG_MAN.shutdown();
        LOG_POOL.shutdown();
        ...
        log.info("LOG_MAN是否完成所有任务："+LOG_MAN.isTerminated());
        log.info("LOG_POOL是否完成所有任务："+LOG_POOL.isTerminated());
    }
